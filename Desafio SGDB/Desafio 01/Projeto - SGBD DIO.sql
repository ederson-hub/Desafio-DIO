/*
Criação das Tabelas
O script a seguir está em SQL padrão e define a estrutura das tabelas conforme o diagrama,
incluindo Chaves Primárias (PK) e Chaves Estrangeiras (FK).
*/
-- Tabela Cliente
...
SELECT  TV.Razao_Social_Vendedor    AS Vendedor_Terceiro,
        F.Razao_Social              AS Fornecedor_Associado
FROM    Terceiro_Vendedor   TV
JOIN    Fornecedor          F ON TV.id_Fornecedor = F.id_Fornecedor
WHERE   TV.id_Fornecedor IS NOT NULL; 
...
FROM    Produto                     P
JOIN    Disponibilizando_Produto    DP  ON P.id_Produto = DP.id_Produto
JOIN    Fornecedor                  F   ON DP.id_Fornecedor = F.id_Fornecedor
JOIN    Produto_has_Estoque         PHE ON P.id_Produto = PHE.id_Produto
JOIN    Estoque                     E   ON PHE.id_Estoque = E.id_Estoque
ORDER BY    Produto, Fornecedor;

-- Relação de nomes dos fornecedores e nomes dos produtos

SELECT  F.Razao_Social  AS Nome_do_Fornecedor,
        P.Descricao     AS Nome_do_Produto
FROM    Fornecedor                  F
JOIN    Disponibilizando_Produto    DP  ON F.id_Fornecedor = DP.id_Fornecedor
JOIN    Produto                     P   ON DP.id_Produto = P.id_Produto
ORDER BY    Nome_do_Fornecedor, Nome_do_Produto;